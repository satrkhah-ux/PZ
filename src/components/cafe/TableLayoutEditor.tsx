...
export function TableLayoutEditor({
  ...
  function onDown(e: React.PointerEvent, name: string) {
    ...
    if (!area || !t) return;
    drag.current = { name, dx: e.clientX - (area.left + (t.x / 100) * area.width), dy: e.clientY - (area.top + (t.y / 100) * area.height), moved: false };
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        ...
        className="relative h-[62vh] min-h-[380px] w-full touch-none rounded-2xl border-2 border-dashed border-border bg-secondary/30"
      >
        ...
        {tables.map((t) => (
          <button
            ...
            className={`absolute flex size-16 -translate-x-0 touch-none flex-col items-center justify-center rounded-xl border-2 text-center text-xs font-bold shadow transition ${
              t.active
                ? t.kind === "outdoor"
